...
class ReportModelTests(TestCase):
  ...
  def test_create_report(self):
    ...
    report = create_report(report={ 
      'trail': trail, 'trailhead': trailhead, 'date_hiked': time.date(), 'day_hiked': 'Th', 'trail_begin': time.time(), 'trail_end': time.time(), 'access': 'P', 'bathroom_status': 'O', 'bathroom_type': 'FP', 'pkg_location': 'P', 'pkg_estimate_begin': 15, 'pkg_estimate_end': 50, 'cars_seen': 30, 'people_seen': 100, 'horses_seen': False, 'dogs_seen': True
    })
